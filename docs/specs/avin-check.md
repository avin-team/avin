...
- Any visitor can report after email OTP verification and private contact capture; Avin account sign-in is not required.
- P0 report forms cover bank/wallet/phone scam, malicious/fake website, and social/game/account-taken-back incidents.
- Shared lifecycle: `DRAFT` → `SUBMITTED` → `UNDER_REVIEW` → optional `CHANGES_REQUESTED` → `SUBMITTED`, or `REJECTED`/`PUBLISHED`; published reports may become `CORRECTED` or `REMOVED`.
- A report without sufficient evidence cannot be published.
- Original evidence is immutable, private, and Admin-only.
- Public evidence is a separately generated derivative that removes unrelated PII and metadata, passes file/malware validation, carries an Avin watermark, and never discloses the original storage URL.
- `RiskIdentifier` normalizes bank, wallet, phone, website, social, and platform-account identifiers for exact matching and grouping.
- Sensitive identifiers are stored full only in approved private storage and are masked in every public projection.
- A Provider-related report starts a verified 48-hour response window. Non-response results in `SUSPENDED_PENDING_REVIEW`, not automatic confirmation of fraud.
- Moderators contact external subjects where a reliable method exists. Urgent or multi-victim risk may be published as “under verification” under the moderation policy.
- P0 has no public comments. Supplemental evidence is a new moderated report.
- Corrections/removals append public history and retain a tombstone unless law requires deletion.
- Only `PUBLISHED` reports count current public statistics. Removed reports remain in audit but leave current totals.
...
- Add protection-specific capabilities: `PROVIDER_REVIEWER`, `RISK_MODERATOR`, `BOND_OPERATOR`, `PROTECTION_MANAGER`, and `SUPER_ADMIN`.
...
- Public statistics include published risk identifier count, current report count, total verified claimed loss, approved reports by day/month/year, Providers by status, and last updated time. They do not include comments.
...
- End-to-end API tests cover anonymous email OTP verification, all three report forms, draft/save/submit, changes requested/resubmit, publish, correct, reject, and remove.
