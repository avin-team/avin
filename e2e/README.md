...
└── src/            # Playwright test runtime source
    ...
    └── tests/
        ...
        └── web/    # Storefront anonymous, OAuth boundary, and session journeys
```

Files ending in `.authenticated.spec.ts` run only when that app's complete set of E2E credentials is configured. Setup projects create `.auth/*.json` and authenticated projects depend on them. Anonymous projects always start with an empty storage state.
...
For the local database, provision both stable accounts once:

```bash
bun run test:e2e:provision
```

The idempotent provisioner writes secrets to `e2e/.env.local`, which is ignored. It reuses existing accounts and never resets an existing account or TOTP secret. Alternatively, copy `e2e/.env.example` to `e2e/.env.local` and provide either or both account sets. A partial credential set fails fast instead of silently skipping tests.
...
- Use `@auth` for auth coverage, `@smoke` for fast critical checks, and `@prod-safe` only for read-only flows.
