...
export const NotificationButton = () => {
  ...
  const unreadQuery = useAdminNotificationUnreadCount(isAuthenticated);
  ...
  return (
    <Popover>
      <PopoverTrigger
        ...
        className="relative inline-flex size-9 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-muted hover:text-foreground cursor-pointer"
      >
      ...
      <PopoverContent align="end" className="w-96 gap-0 overflow-hidden p-0">
        <div className="flex items-start justify-between gap-3 border-b border-border/60 px-4 py-3">
          <div>
            <h2 className="text-base font-semibold">Thông báo Admin</h2>
            <p
              className={cn(
                "text-xs font-medium",
                unreadCount > 0
        ...
        <div className="max-h-95 space-y-1 overflow-y-auto p-2">
          ...
          {recentNotifications.map((notification) => (
            <a
              className={cn(
                "block rounded-2xl p-3 transition-colors hover:bg-muted cursor-pointer",
