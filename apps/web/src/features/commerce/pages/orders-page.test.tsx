...
describe("OrdersPage", () => {
  ...
  it("renders mixed OrderItem states with each item's timeline and actions", () => {
    mocks.ordersQuery.data = [order];

    render(<OrdersPage />);

    expect(screen.getByText(/Studio Avin/u)).toBeInTheDocument();
    expect(screen.getByText("Chờ Seller tiếp nhận")).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("button", { name: "Hủy trước khi Seller bắt đầu" })
    ).toBeInTheDocument();
  });
});
