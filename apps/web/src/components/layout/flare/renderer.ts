...
import type { FlarePlacement, Point } from "./pipeline";
import {
  backingDimensions,
  canvasRaster,
  FlarePipeline,
...
type RenderSize = Readonly<{ dpr: number; height: number; width: number }>;
...
export const createRenderer = ({
  ...
  const applySize = async (size: RenderSize, generation: number) => {
    ...
    try {
      ...
      if (rasterAbort === controller) {
        rasterAbort = undefined;
      }
    }
  ...
  const frameLoop = (now: number) => {
    ...
    guard(() => {
      ...
      if (
        now - lastRender < FRAME_INTERVAL_MS ||
        !placement ||
        !activePipeline
      ) {
        return;
  ...
  const initialize = async () => {
    ...
    const nextGpu = await init({ label: "avin-flare-logo" });
    if (disposed) {
      try {
        nextGpu.dispose();
      } catch {
