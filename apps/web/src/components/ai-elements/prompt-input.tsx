import {
  ...
  CommandList,
  CommandSeparator,
} from "@avin/ui/components/command";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
import {
  ...
  HoverCardContent,
  HoverCardTrigger,
} from "@avin/ui/components/hover-card";
import {
  InputGroup,
  InputGroupAddon,
  InputGroupButton,
...
type PromptInputActionSelectEvent = Parameters<
...
export const PromptInputActionAddAttachments = ({
  ...
  const handleSelect = useCallback(
    (e: PromptInputActionSelectEvent) => {
      e.preventDefault();
...
export const PromptInputActionAddScreenshot = ({
  ...
  const handleSelect = useCallback(
    async (event: PromptInputActionSelectEvent) => {
      onSelect?.(event);
  ...
  const addLocal = useCallback(
    (fileList: File[] | FileList) => {
      ...
      if (incoming.length && accepted.length === 0) {
        onError?.({
          code: "accept",
          message: "Không có tệp phù hợp với định dạng được hỗ trợ.",
        });
        return;
      }
      ...
      if (accepted.length > 0 && sized.length === 0) {
        onError?.({
          code: "max_file_size",
          message: "Tất cả tệp đều vượt quá dung lượng cho phép.",
        });
        return;
      }
      ...
      setItems((prev) => {
        ...
        if (typeof capacity === "number" && sized.length > capacity) {
          onError?.({
            code: "max_files",
            message: "Có quá nhiều tệp. Một số tệp chưa được thêm.",
          });
        }
      ...
      try {
        ...
        if (result instanceof Promise) {
          try {
            ...
            onError?.({
              code: "submit",
              message: "Không thể gửi lượt preview. Vui lòng thử lại.",
            });
          }
        } else {
        ...
        onError?.({
          code: "submit",
          message: "Không thể chuẩn bị tệp đính kèm. Vui lòng thử lại.",
        });
      }
    },
    [usingProvider, controller, files, onSubmit, onError, clear]
  );
...
export const PromptInputSubmit = ({
  ...
  return (
    <InputGroupButton
      aria-label={isGenerating ? "Dừng" : "Gửi"}
      className={cn(className)}
      onClick={handleClick}
      size={size}
