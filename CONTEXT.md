...
An unauthenticated person exploring Avin's public marketplace. A `Visitor` may use the `Service Advisor` after recording `Advisor Consent` and receive recommendations, but must become an authenticated `User` before adding an offering to the `Cart` or purchasing it. _Avoid_: Guest, Customer.

### Service Need

A Buyer's non-sensitive description of the outcome they want, including relevant context, supporting `Advisory Attachment`s, budget, and timing. It guides marketplace recommendations but is not a professional diagnosis and never includes passwords, OTPs, access tokens, payment details, or identity-verification documents. _Avoid_: Problem, Diagnosis.

### Service Advisor

Avin's pre-purchase conversational guide that clarifies a `Service Need` and produces an `Advisor Recommendation` when the relevant Playbook's completion conditions are met. It advises discovery but does not perform the service or replace the Buyer's final purchase choice. _Avoid_: Support Chat, Order Chat, Diagnostic Bot.

### Advisory Session

A resumable pre-purchase conversation between a `Visitor` or `User` and the `Service Advisor`, including its messages, `Advisory Attachment`s, and successive `Advisor Recommendation`s; it addresses one active `Service Need`, and the newest eligible result is current while earlier results remain conversation history. A Visitor's session is identified without creating an account and expires after 24 hours of inactivity; linking it to a User requires that person's explicit choice and extends retention to 30 days after its last activity, while the participant may delete the session earlier at any time. _Avoid_: Order Chat, Support Ticket.

### Advisory Attachment

A private JPEG, PNG, or WebP image that a `Visitor` or `User` supplies as visual context for a `Service Need`. It belongs only to its `Advisory Session`, shares that session's retention, and becomes a `Checkout Attachment` only when an authenticated User explicitly selects and confirms the transfer. _Avoid_: Order File, Public Media.

### Advisory Summary

A User-reviewed, editable summary created only after the participant chooses an `Advisor Recommendation` and asks to continue to its Listing. It becomes a `Buyer Checkout Note` only when the User explicitly chooses to copy and confirm it during Checkout; the full transcript is never transferred, and only individually selected `Advisory Attachment`s may accompany the summary.

### Advisor Playbook

Versioned, Admin-owned marketplace knowledge for one Sub-Category that defines relevant need signals, clarification questions, exclusion conditions, and the minimum information required for an `Advisor Recommendation`. A Playbook moves from `DRAFT` to `PUBLISHED` or `ARCHIVED`, each Sub-Category has at most one published version, and an `Advisory Session` pins the published version when it begins evaluating that Sub-Category. _Avoid_: System Prompt, Diagnosis Script.

### Advisor Recommendation

An explainable result containing no more than three `SERVICE` Listings that were purchasable when recommended, with a suggested `ServicePackage` when the available information supports one and no more than one result per Seller when equally eligible alternatives exist. It exists only after the pinned `Advisor Playbook` is complete; Service Need fit ranks first, budget, scope, and timing constrain eligibility, marketplace reputation only breaks otherwise comparable results, and current availability is revalidated before any Buyer action. _Avoid_: Diagnosis, Sponsored Result.

### Advisor Feedback

A participant's positive or negative evaluation of an `Advisor Recommendation`, with an optional reason. It shares no transcript with Admin unless the participant explicitly submits the conversation, and each `Advisory Attachment` requires separate consent before it is included.

### Advisor Consent

A versioned acknowledgment recorded before a participant's first `Advisory Session` that explains AI-provider processing, Avin's retention, and the prohibition on submitting credentials or other secrets. It is distinct from the Seller Agreement and from consent to share a transcript or individual `Advisory Attachment` as `Advisor Feedback`.
...
- **`SERVICE`**: Manual digital service requiring Seller fulfillment for a Buyer's requested outcome (e.g., account unlock, custom setup).
...
- **Commercial & Templates**: Each **Sub-Category** holds an Admin-configured `commissionRatePercent`, a reusable default `WarrantyPolicy` template, and `WarrantyBounds` (min/max duration in hours). For a `SERVICE`, the template initializes the first `ServicePackage` of a new Listing, which may tailor its own policy; a package with a timed warranty must remain within the selected Sub-Category’s bounds, while explicit `NO_WARRANTY` is also valid. A `COURSE` retains its existing Listing-level commercial contract. Parent Categories are purely organizational and do not store commission rates.
...
A private, immutable file shared as a chat attachment, `Checkout Attachment`, or fulfillment deliverable within an `Order`. Access is limited to that Order's `User`, `Seller`, and an authorized Admin. A submitted file cannot be overwritten or deleted by a participant; an Admin may quarantine or redact it from normal views while preserving the original and its audit trail.
...
An optional free-form description, limited to 1,000 characters, that a Buyer supplies for one selected Listing during Checkout and that becomes part of the resulting `OrderItem`. It gives the Seller fulfillment context but is not part of the Listing or ServicePackage contract; an `Advisory Summary` may populate it only after explicit Buyer review and confirmation. _Avoid_: Service Input, Order Custom Input.

### Checkout Attachment

An optional private image that a Buyer supplies for one selected Listing during Checkout and that becomes an `OrderFile` attached to the resulting `OrderItem`. A Buyer may provide at most five JPEG, PNG, or WebP Checkout Attachments per selected Listing, with a maximum size of 10 MB each; a selected `Advisory Attachment` may populate one only after explicit Buyer preview and confirmation.
...
3. **`ListingAggregate`**: `Listing` + `ServicePackage`s for `SERVICE` Listings (including their `WarrantyPolicy`) + `Category`
4. **`OrderAggregate`**: `Order` + `OrderItem`s (including their optional `Buyer Checkout Note`) + per-item `EscrowHold`s + `DeliverySubmission`s + `OrderChat` (Messages) + `OrderFile`s (including `Checkout Attachment`s)
