...
- Phosphor icon imports MUST use the `Icon` suffix (e.g., `CheckIcon`, `ArrowRightIcon`, `SpinnerIcon`, `StorefrontIcon`). The bare/unsuffixed names (e.g., `Check`, `ArrowRight`, `Spinner`) are deprecated by `@phosphor-icons/react` and produce TypeScript deprecation warnings. The only exception is the type import: `import type { Icon } from "@phosphor-icons/react"`. Confidence: 0.90
- When performing bulk codebase-wide rename/refactor operations (e.g., icon name migrations, symbol renames), replacements must be scoped to code identifiers, imports, and JSX elements only. Never let global regex replacements touch user-facing text strings — labels, aria-labels, button text, placeholder strings, toast messages, or any content rendered to the user. Regex like `\bX\b` is too aggressive when `X` is a short token that appears inside natural-language words (e.g., Vietnamese "Xóa" → "Xóa"). Confidence: 0.75
